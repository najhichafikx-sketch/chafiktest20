...
export async function initDB() {
  ...
  try {
    ...
    await sql(`CREATE INDEX IF NOT EXISTS idx_prompts_slug ON prompts(slug)`);
    await sql(`CREATE INDEX IF NOT EXISTS idx_prompts_status ON prompts(status)`);
    await sql(`CREATE INDEX IF NOT EXISTS idx_prompts_tool ON prompts(tool)`);
    return true;
  } catch (err) {
...
export async function updatePrompt(id, data) {
  if (dbAvailable()) {
    ...
    if (sql) {
      ...
      vals.push(Number(id));
      await sql`UPDATE prompts SET ${sql(sets.join(', '), ...vals)}, updated_at = NOW() WHERE id = ${Number(id)}`;
      return;
    }
  }
  const prompts = loadPrompts();
