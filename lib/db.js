...
const BLOG_DETAIL_COLUMNS = `id, slug, title, excerpt, content, category, tags, author, meta_description, seo_title, keywords, reading_time, status, published_at, created_at, updated_at, external_link, external_link_label, has_image, featured_image`;
...
export async function getBlogPostById(id) {
  if (dbAvailable()) {
    try {
      ...
      let rows;
      if (!isNaN(numId)) {
        rows = await sql.query(`SELECT ${BLOG_DETAIL_COLUMNS} FROM blog_posts WHERE id = $1`, [numId]);
      } else {
        rows = await sql.query(`SELECT ${BLOG_DETAIL_COLUMNS} FROM blog_posts WHERE slug = $1`, [id]);
      }
...
export async function createBlogPost(data) {
  ...
  if (dbAvailable()) {
    ...
    if (sql) {
      ...
      try {
        const posts = loadBlogPosts();
        const existing = posts.findIndex(p => String(p.id) === String(syncPost.id) || p.slug === syncPost.slug);
        if (existing >= 0) posts[existing] = { ...posts[existing], ...syncPost };
        else posts.push(syncPost);
        saveBlogPosts(posts);
      } catch {}
      return rows[0];
    }
