...
const BLOG_LIST_COLUMNS = `id, slug, title, excerpt, category, tags, author, meta_description, seo_title, keywords, reading_time, status, published_at, created_at, updated_at, external_link, external_link_label, has_image, featured_image`;
...
export async function getBlogPostFeaturedImage(slug) {
  ...
  try {
    const { data } = await sb.from('blog_posts')
      .select('featured_image, has_image')
      .eq('slug', slug)
      .single();
    return data;
  } catch { return null; }
