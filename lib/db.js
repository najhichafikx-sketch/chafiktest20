  ...
  if (dbAvailable()) {
    ...
    if (sql) {
      const rows = await sql`
        INSERT INTO blog_posts (slug, title, excerpt, content, category, tags, author, featured_image, meta_description, seo_title, keywords, reading_time, status)
        VALUES (${clean.slug}, ${clean.title}, ${clean.excerpt || ''}, ${clean.content || ''}, ${clean.category || 'General'}, ${clean.tags || []}, ${clean.author || 'Chafiktech Ai'}, ${clean.featured_image || ''}, ${clean.meta_description || ''}, ${clean.seo_title || ''}, ${clean.keywords || []}, ${clean.reading_time || 5}, ${clean.status || 'draft'})
        RETURNING id
      `;
      return rows[0];
    }
