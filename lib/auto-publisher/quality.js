...
function checkSEO(article) {
  ...
  else if (article.title.length >= 40 && article.title.length <= 70) score += 8;
  else issues.push(`Title length ${article.title.length} not in 50-60 range`);

  // Meta description
  if (article.metaDescription.length >= 140 && article.metaDescription.length <= 160) score += 15;
  else if (article.metaDescription.length >= 100 && article.metaDescription.length <= 200) score += 8;
  ...
  // H2 count (6-8 now for long-form)
  const h2Count = (article.content.match(/<h2/g) || []).length;
  if (h2Count >= 6 && h2Count <= 10) score += 20;
  else if (h2Count >= 4) score += 10;
  else issues.push(`Only ${h2Count} H2 sections (need 6-8)`);

  // FAQ present (4-6 now)
  if (article.faqs && article.faqs.length >= 4) score += 12;
  else if (article.faqs && article.faqs.length >= 2) score += 6;
  else issues.push('No FAQ section');

  // Tags
  if (article.tags && article.tags.length >= 4) score += 10;
  else if (article.tags && article.tags.length >= 2) score += 5;

  // Hashtags bonus
  if (article.hashtags && article.hashtags.length > 0) score += 8;

  // KeyFacts bonus (featured snippets)
  if (article.keyFacts && article.keyFacts.length >= 3) score += 10;
