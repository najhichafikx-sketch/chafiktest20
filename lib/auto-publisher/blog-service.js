...
export async function createBlogPost(article, metadata = {}) {
  try {
    ...
    const { data, error } = await sb.from('blog_posts')
      .upsert(post, { onConflict: 'slug,locale' })
      .select('id, slug')
      .single();
...
function buildPostRow(article, metadata, language, bundle = null) {
  ...
  // CLEAN URLs: use base slug without language suffix
  // Multi-language versions are stored in separate rows (base_slug + locale columns)
  const localizedSlug = baseSlug;
