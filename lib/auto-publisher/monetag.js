...
export function injectMonetagAds(html) {
  ...
  // 4. NEW (2026-06-12): Top banner before first H2 - high CTR
  result = injectBeforeFirstH2(result, getBannerAd());

  // 5. NEW (2026-06-12): SmartLink after 5th paragraph - mid-article click
  result = injectAfterParagraph(result, 5, getSmartLinkAd());
...
// NEW: Inject banner BEFORE the first H2 (high visibility)
function injectBeforeFirstH2(html, adHtml) {
  return html.replace(/(<h2[^>]*>)/, `${adHtml}\n$1`);
}

// NEW: SmartLink (high-CTR text ad that users actually click)
function getSmartLinkAd() {
  return `
<div class="monetag-slot monetag-smartlink" data-zone="smartlink">
  <a href="https://www.effectivecpmnetwork.com/kvnw38jp?key=d0cba4ffa97061f6ee503168678dac31"
     target="_blank" rel="sponsored noopener"
     style="display:block;padding:16px 20px;background:linear-gradient(135deg,#4f46e5,#7c3aed);
            color:#fff;text-align:center;border-radius:8px;text-decoration:none;
            font-weight:700;font-size:15px;margin:24px 0;box-shadow:0 4px 12px rgba(99,102,241,0.3);">
    💎 Continue Reading: See Live Results, Stats &amp; Expert Analysis →
  </a>
</div>`.trim();
}
