...
function buildMatchDataSection(trend) {
  ...
  if (md.team1 && md.team2 && md.venue) {
    // Build TV channels string
    let tvInfo = `- International broadcasters: beIN Sports (MENA), SuperSport (Africa), ESPN/Star+ (Latin America), Optus Sport (Australia)`;
    if (md.team1 === 'USA' || md.team2 === 'USA') {
      tvInfo = `- Broadcasters: Fox/FS1/Telemundo (USA), SBS/MBC/KBS (South Korea), beIN Sports (MENA), BBC iPlayer (UK)`;
    } else if (md.team1 === 'Morocco' || md.team2 === 'Morocco') {
      tvInfo = `- Broadcasters: Arryadia/SNRT/beIN Sports (Morocco), ${md.team1 === 'Morocco' ? md.team2 : md.team1} local channels, beIN Sports (MENA), SuperSport (Africa), Canal+ (France)`;
    } else if (md.team1 === 'Brazil' || md.team2 === 'Brazil') {
      tvInfo += `, Globo/SporTV (Brazil)`;
    } else if (md.team1 === 'Argentina' || md.team2 === 'Argentina') {
      tvInfo += `, TyC Sports/Telefe (Argentina)`;
    } else if (md.team1 === 'England' || md.team2 === 'England' || md.team1 === 'Scotland' || md.team2 === 'Scotland') {
      tvInfo += `, BBC One/ITV (UK)`;
    } else if (md.team1 === 'France' || md.team2 === 'France') {
      tvInfo += `, TF1/M6/beIN Sports (France)`;
    } else if (md.team1 === 'Germany' || md.team2 === 'Germany') {
      tvInfo += `, ARD/ZDF/MagentaTV (Germany)`;
    } else if (md.team1 === 'Spain' || md.team2 === 'Spain') {
      tvInfo += `, RTVE/Movistar Plus+ (Spain)`;
    }
...
- Kick-off: ${md.timeUTC ? md.timeUTC + ' UTC' : 'TBC'}
...
CRITICAL RULES:
1. You MUST mention the exact date, kick-off time, venue, and city in your article
2. You MUST mention both teams' names multiple times
3. You MUST list the TV channels broadcasting this match in each team's home country
4. You MUST describe the venue and its significance
5. If this match has NOT happened yet, write it as a PREVIEW (not a recap)
6. If this match has already happened, you may briefly summarize but DO NOT fabricate scores or specific plays
7. Include match context: group standings implications, tournament stage
8. Use team-specific keywords naturally (e.g. "Morocco national team World Cup 2026", "Scotland vs Morocco fixture")
`;
  }
...
- OG Title: 50-60 characters, optimized for social sharing (Facebook, Twitter, LinkedIn)
- OG Description: 150-155 characters, optimized for social preview with a compelling hook + CTA
...
- 20 long-tail keywords (3+ word phrases users actually search for) — provide all 20 in the JSON
- 8-10 related search terms (People Also Ask / autocomplete style) — provide them in the JSON
...
- Include a "Match Details" / "تفاصيل المباراة" / "Détails du match" callout box with: Date, Time, Venue, City, Stadium Capacity, Tournament Stage, TV Channels (local + international)` : ''}
...
### Internal Linking
- Link to 3 related articles within the content using natural anchor text
- Link to 2 category pages (e.g. "/blog/category/sports", "/blog/category/ai")
- Include 1 link to the homepage ("/")
- All internal links must use relative paths and descriptive anchor text${isWcTopic ? `
- Link to match schedule page and team guide pages` : ''}

### Structured Data (Schema.org)
${isWcTopic ? `Include a SportsEvent schema in the HTML for the match with:
- @type: SportsEvent
- name: "${md?.team1 || ''} vs ${md?.team2 || ''}" 
- startDate: the match date and time in ISO 8601 format
- location: { @type: Place, name: "${md?.venue || ''}", address: { @type: PostalAddress, addressLocality: "${md?.city || ''}" } }
- sport: "Soccer"
- competitor: [{ @type: SportsTeam, name: "${md?.team1 || ''}" }, { @type: SportsTeam, name: "${md?.team2 || ''}" }]
` : `Include an Article schema in the HTML:
- @type: Article
...

{
  ...
  "ogTitle": "OG title for social sharing (50-60 chars)",
  "ogDescription": "OG description for social preview (150-155 chars)",
  ...
  "content": "Full HTML body with internal links and schema markup. Use <h2>, <h3>, <p>, <ul>, <ol>, <li>, <strong>, <em>, <table>, <tr>, <th>, <td>, <a>, <blockquote> tags. MUST start with <p>. Minimum 1500 words.",
  "primaryKeyword": "the single most important search phrase",
  "secondaryKeywords": ["kw1", "kw2", "kw3", "kw4", "kw5"],
  "longTailKeywords": ["exactly 20 long-tail keyword phrases people search for"],
  "relatedSearchTerms": ["8-10 related search queries"],
  "tags": ["tag1", "tag2", "tag3", "tag4", "tag5", "tag6"],
  "category": "One of the categories above",
  "internalLinks": ["/related-article-slug", "/blog/category/sports", "/"],
  "faqs": [
